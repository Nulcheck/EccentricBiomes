...
public class WorldGenModLake extends WorldGenerator {
	public static Block block;
	public static Block blockOutline;

	public WorldGenModLake(Block block, Block blockOutline) {
		WorldGenModLake.block = block;
		WorldGenModLake.blockOutline = blockOutline;
	}
				...
				for (j2 = 0; j2 < 16; ++j2) {
					for (j1 = 0; j1 < 8; ++j1) {
						if (aboolean[(i1 * 16 + j2) * 8 + j1]) {
							world.setBlock(x + i1, y + j1, z + j2, j1 >= 4 ? Blocks.air : block, 0, 2);
						}
					}
				}
			}
					...
					for (j2 = 0; j2 < 16; ++j2) {
						for (j1 = 0; j1 < 8; ++j1) {
							...
							if (flag && (j1 < 4 || rand.nextInt(2) != 0)
									&& world.getBlock(x + i1, y + j1, z + j2).getMaterial().isSolid()) {
								world.setBlock(x + i1, y + j1, z + j2, blockOutline, 0, 2);
							}
						}
					}
				}
