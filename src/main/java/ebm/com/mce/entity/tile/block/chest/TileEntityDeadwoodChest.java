...
public class TileEntityDeadwoodChest extends TileEntity implements IInventory {
	private ItemStack[] chestContents = new ItemStack[36];
	public boolean adjacentChestChecked;
	public TileEntityDeadwoodChest adjacentChestZNeg;
	public TileEntityDeadwoodChest adjacentChestXPos;
	public TileEntityDeadwoodChest adjacentChestXNeg;
	public TileEntityDeadwoodChest adjacentChestZPos;
	...
	public TileEntityDeadwoodChest() {
		this.cachedChestType = -1;
	}

	@SideOnly(Side.CLIENT)
	public TileEntityDeadwoodChest(int i) {
	...
	public String getInventoryName() {
		return this.hasCustomInventoryName() ? this.customName : "container.deadChest";
	}
	...
	private void adjacentChest(TileEntityDeadwoodChest te, int i) {
		if (te.isInvalid()) {
			this.adjacentChestChecked = false;
	...
	public void checkForAdjacentChests() {
		if (!this.adjacentChestChecked) {
			...
			if (this.validateChest(this.xCoord - 1, this.yCoord, this.zCoord)) {
				this.adjacentChestXNeg = (TileEntityDeadwoodChest) this.worldObj.getTileEntity(this.xCoord - 1,
						this.yCoord, this.zCoord);
			}

			if (this.validateChest(this.xCoord + 1, this.yCoord, this.zCoord)) {
				this.adjacentChestXPos = (TileEntityDeadwoodChest) this.worldObj.getTileEntity(this.xCoord + 1,
						this.yCoord, this.zCoord);
			}

			if (this.validateChest(this.xCoord, this.yCoord, this.zCoord - 1)) {
				this.adjacentChestZNeg = (TileEntityDeadwoodChest) this.worldObj.getTileEntity(this.xCoord, this.yCoord,
						this.zCoord - 1);
			}

			if (this.validateChest(this.xCoord, this.yCoord, this.zCoord + 1)) {
				this.adjacentChestZPos = (TileEntityDeadwoodChest) this.worldObj.getTileEntity(this.xCoord, this.yCoord,
						this.zCoord + 1);
			}
	...
	private boolean validateChest(int x, int y, int z) {
		if (this.worldObj == null) {
			...
			return block instanceof DeadwoodChest && ((DeadwoodChest) block).meta == this.getChestType();
		}
	}
	...
	public void closeInventory() {
		if (this.getBlockType() instanceof DeadwoodChest) {
			--this.numPlayersUsing;
			this.worldObj.addBlockEvent(this.xCoord, this.yCoord, this.zCoord, this.getBlockType(), 1,
					this.numPlayersUsing);
			this.worldObj.notifyBlocksOfNeighborChange(this.xCoord, this.yCoord, this.zCoord, this.getBlockType());
	...
	public int getChestType() {
		if (this.cachedChestType == -1) {
			if (this.worldObj == null || !(this.getBlockType() instanceof DeadwoodChest)) {
				return 0;
			}

			this.cachedChestType = ((DeadwoodChest) this.getBlockType()).meta;
		}
