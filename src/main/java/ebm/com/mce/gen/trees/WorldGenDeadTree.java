...
public class WorldGenDeadTree extends WorldGenAbstractTree {
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		if (block != mod_ebm.deadGrass) {
			...
			for (int i = 0; i < base; i++) {
				buildBlock(world, x, y + h, z, mod_ebm.deadLog, 0);
				h++;
			}
	...
	public void generateTop(World world, int x, int y, int z) {
		buildBlock(world, x, y, z, mod_ebm.deadLog, 0);
	}

	public void generateBranch(World world, Random rand, int x, int y, int z, int p) {
		buildBlock(world, x, y, z, mod_ebm.deadLog, 0);
		buildBlock(world, x, y + 1, z, mod_ebm.deadLog, 0);
	}
